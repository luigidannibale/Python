...
class player :
    def __init__ (self, number, points, string):
        self.number,  self.points, self.string = number, points, string
...
def challenge (n1,player1_txt,n2,player2_txt,k):
    ...
    p1 = player(n1,0,map(ord, player1_txt.replace("\t","").replace(" ","")))
    p2 = player(n2,0,map(ord, player2_txt.replace("\t","").replace(" ","")))    
    
    #### Cylce that executes the challenge between every char of the two strings (p1.string, p2.string){
    assert (len(p1.string) == len(p2.string))
